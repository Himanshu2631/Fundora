...
const emailFrom = process.env.EMAIL_FROM || "Sahayata <noreply@sahayata.com>";
        ...
        <div class="wrapper">
          <div class="container">
            <div class="header">
              ...
              <span class="brand-name">SAHAYATA</span>
            </div>
            <div class="content">
            ...
            <div class="footer">
              <p>Sahayata operates as a provably fair giving & audited sweepstakes platform.</p>
              <p>This message was sent to you as part of your system transactional receipts.</p>
              <p>&copy; 2026 Sahayata. All rights reserved.</p>
            </div>
          </div>
        </div>
      </body>
...
export async function sendSystemUpdateEmail(to, { userName, updateTitle, updateDetails, actionUrl }) {
  ...
  const subject = `[Sahayata] System Update: ${updateTitle}`;
  const bodyHtml = `
    <p>Hello ${userName || "Valued Philanthropist"},</p>
    <p>We want to share a recent update regarding the Sahayata platform:</p>
...
export async function sendDrawResultEmail(
  ...
  const subject = `[Sahayata] Draw Results: ${drawName}`;
  ...
  const bodyHtml = `
    ...
    <div style="background-color: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 8px; padding: 16px; margin: 16px 0; font-size: 13.5px;">
      ...
      <p style="margin: 4px 0; color: #B2C0B9;"><strong>Status:</strong> ${isWinner
  ...
  const html = getBaseEmailHtml({
    ...
    ctaUrl: actionUrl || `${getDefaultAppUrl()}/dashboard/draws`,
  });
...
export async function sendWinnerAlertEmail(
  ...
  const subject = `🚨 WINNER ALERT: You won the ${drawName} on Sahayata!`;
...
export async function sendWinnerAlertNotificationEmail(
  ...
  const subject = "Congratulations! You Won a Sahayata Reward Draw";
  ...
  const bodyHtml = `
    ...
    <p>Thank you for being an active member of Sahayata. Your continued support drives real, audited giving impact worldwide.</p>
  `;
