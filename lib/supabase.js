...
const isPlaceholderKey = (key) => {
  ...
  return (
    !k ||
    k === "placeholder-anon-key" ||
    k.includes("your-anon-key") ||
    k.includes("your-publishable-key") ||
    k === "your-key-here"
  );
};
...
const isPlaceholderUrl = (url) => {
  ...
  return (
    ...
    u.includes("placeholder.supabase")
  );
};

export const getSupabaseCredentials = () => {
  const url = sanitizeEnv(process.env.NEXT_PUBLIC_SUPABASE_URL || "");
  const rawAnon = sanitizeEnv(process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || "");
  const rawPub = sanitizeEnv(process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || "");

  let activeKey = "";
  if (!isPlaceholderKey(rawPub)) {
    activeKey = rawPub;
  } else if (!isPlaceholderKey(rawAnon)) {
    activeKey = rawAnon;
  } else {
    activeKey = rawPub || rawAnon || "";
  }

  const isPlaceholder = isPlaceholderUrl(url) || isPlaceholderKey(activeKey);
...
export const isPlaceholder = getSupabaseCredentials().isPlaceholder;
...
const getMockUsers = () => {
  ...
  try {
    ...
    if (stored) {
      ...
      // Seed default reviewer admin and user for local testing
      users = [
        {
          id: "mock-uid-reviewer-admin",
          email: "admin@sahayata.demo",
          password: "Admin@123",
          full_name: "Reviewer Admin"
        },
        {
          id: "mock-uid-user",
          email: "user@sahayata.com",
          password: "user",
          full_name: "User Demo"
        }
      ];
    ...
    // Clean up old quick login admin account
    users = users.filter(u => u.email !== "admin@fundora.com");

    // Ensure new reviewer admin account exists
    if (!users.some(u => u.email === "admin@sahayata.demo" || u.email === "admin@fundora.demo")) {
      users.push({
        id: "mock-uid-reviewer-admin",
        email: "admin@sahayata.demo",
        password: "Admin@123",
...
const getMockDrawParticipation = () => {
  if (typeof window === "undefined") return [];
  try {
    return JSON.parse(localStorage.getItem("fundora-mock-draw-participation") || "[]");
  } catch {
    return [];
  }
};

const saveMockDrawParticipation = (participation) => {
  if (typeof window === "undefined") return;
  localStorage.setItem("fundora-mock-draw-participation", JSON.stringify(participation));
  setCookie("fundora-mock-draw-participation", JSON.stringify(participation));
};
...
const createMockClient = () => {
  console.warn("⚠️ Sahayata Client: Using Mock Supabase client due to placeholder credentials.");
  ...
  return {
    auth: {
      ...
      resetPasswordForEmail: async (email, options) => {
        ...
        console.log(`[Mock Supabase] resetPasswordForEmail called for ${email} with options:`, options);
          ...
          return {
            subscribe: () => {
              ...
              if (typeof window !== "undefined") {
                window.addEventListener(`mock-realtime-${name}`, handler);
              }
              return {
                unsubscribe: () => {
                  if (typeof window !== "undefined") {
                    window.removeEventListener(`mock-realtime-${name}`, handler);
                  }
                }
              };
            }
    ...
    from: (table) => {
        ...
        try {
          ...
          if (cookieVal) {
            try {
              ...
              const index = subs.findIndex(s => s.id === cookieSub.id || s.stripe_subscription_id === cookieSub.stripe_subscription_id);
              if (index === -1) {
                subs.push(cookieSub);
                ...
                if (
                  existing.plan_type !== cookieSub.plan_type ||
                  existing.status !== cookieSub.status ||
                  existing.stripe_subscription_id !== cookieSub.stripe_subscription_id ||
                  existing.stripe_price_id !== cookieSub.stripe_price_id ||
                  existing.renewal_date !== cookieSub.renewal_date
                ) {
                  subs[index] = { ...existing, ...cookieSub };
                  localStorage.setItem("fundora-mock-subscriptions", JSON.stringify(subs));
                }
              }
            } catch (e) {
      ...
      const saveMockScores = (scores) => {
        ...
        setCookie("fundora-mock-scores", JSON.stringify(scores));
      };
      ...
      const getMockDraws = () => {
        ...
        try {
          ...
          if (stored) {
            ...
            setCookie("fundora-mock-draws", JSON.stringify(draws));
          }

          const cookieVal = getCookie("fundora-mock-draws");
          if (cookieVal) {
            try {
              const cookieDraws = JSON.parse(cookieVal);
              let updated = false;
              cookieDraws.forEach(cd => {
                const index = draws.findIndex(d => d.id === cd.id);
                if (index === -1) {
                  draws.push(cd);
                  updated = true;
                } else {
                  const local = draws[index];
                  if (
                    local.status !== cd.status ||
                    JSON.stringify(local.winning_numbers) !== JSON.stringify(cd.winning_numbers) ||
                    local.title !== cd.title ||
                    local.prize !== cd.prize
                  ) {
                    draws[index] = { ...local, ...cd };
                    updated = true;
                  }
                }
              });
              ...
              console.warn("Failed to parse mock draws cookie:", e);
            }
          }
          return draws;
        } catch {
      ...
      const getMockDrawEntries = () => {
        if (typeof window === "undefined") return [];
        try {
          let entries = JSON.parse(localStorage.getItem("fundora-mock-draw-entries") || "[]");
          const cookieVal = getCookie("fundora-mock-draw-entries");
          if (cookieVal) {
            try {
              const cookieEntries = JSON.parse(cookieVal);
              let updated = false;
              cookieEntries.forEach(ce => {
                const index = entries.findIndex(e => e.id === ce.id);
                if (index === -1) {
                  entries.push(ce);
                  updated = true;
                } else {
                  const local = entries[index];
                  if (JSON.stringify(local.numbers) !== JSON.stringify(ce.numbers) || local.ticket_number !== ce.ticket_number) {
                    entries[index] = { ...local, ...ce };
                    updated = true;
                  }
                }
              });

              if (cookieEntries.length < entries.length) {
                entries = entries.filter(e => cookieEntries.some(ce => ce.id === e.id));
                updated = true;
              }
              ...
              console.warn("Failed to parse mock draw entries cookie:", e);
            }
          }
          return entries;
        } catch {
      ...
      const saveMockDrawEntries = (entries) => {
        if (typeof window === "undefined") return;
        localStorage.setItem("fundora-mock-draw-entries", JSON.stringify(entries));
        setCookie("fundora-mock-draw-entries", JSON.stringify(entries));
      };
      ...
      const getMockClaims = () => {
        ...
        try {
          ...
          if (stored) {
            ...
            setCookie("fundora-mock-winner-claims", JSON.stringify(claims));
          }

          const cookieVal = getCookie("fundora-mock-winner-claims");
          if (cookieVal) {
            try {
              const cookieClaims = JSON.parse(cookieVal);
              let updated = false;
              cookieClaims.forEach(cc => {
                const index = claims.findIndex(c => c.id === cc.id);
                if (index === -1) {
                  claims.push(cc);
                  updated = true;
                } else {
                  const local = claims[index];
                  if (local.status !== cc.status || JSON.stringify(local.status_history) !== JSON.stringify(cc.status_history)) {
                    claims[index] = { ...local, ...cc };
                    updated = true;
                  }
                }
              });
              ...
              console.warn("Failed to parse mock winner claims cookie:", e);
            }
          }
          return claims;
        } catch {
      ...
      return {
        select: (fields) => {
          ...
          if (table === "profiles") {
            ...
            matchedData = [
              ...
              ...mockUsers.map(u => ({
                ...
                pref_system_updates: u.pref_system_updates !== false,
                pref_draw_results: u.pref_draw_results !== false,
                pref_winner_alerts: u.pref_winner_alerts !== false,
              })),
            ...
            matchedData = [
              { id: "sp-scout-monthly", stripe_price_id: "price_scout_monthly", plan_name: "scout", billing_cycle: "monthly", amount: 499.00 },
              { id: "sp-scout-yearly", stripe_price_id: "price_scout_yearly", plan_name: "scout", billing_cycle: "yearly", amount: 4788.00 },
              { id: "sp-advocate-monthly", stripe_price_id: "price_advocate_monthly", plan_name: "advocate", billing_cycle: "monthly", amount: 1299.00 },
              { id: "sp-advocate-yearly", stripe_price_id: "price_advocate_yearly", plan_name: "advocate", billing_cycle: "yearly", amount: 11988.00 },
              { id: "sp-builder-monthly", stripe_price_id: "price_builder_monthly", plan_name: "builder", billing_cycle: "monthly", amount: 4999.00 },
              { id: "sp-builder-yearly", stripe_price_id: "price_builder_yearly", plan_name: "builder", billing_cycle: "yearly", amount: 47988.00 }
            ];
          ...
          } else if (table === "draw_participation") {
            matchedData = getMockDrawParticipation();
          }
          ...
          const makeChain = (data) => {
            const chain = {
              ...
              then: async (resolve) => resolve({ data, error: null, count: data.length })
            };
            return chain;
          };
        ...
        insert: (data) => {
          if (table === "subscriptions") {
            ...
            if (typeof window !== "undefined") {
              window.dispatchEvent(new CustomEvent('mock-realtime-subscriptions', { detail: { new: newSub } }));
            }
            ...
            saveMockScores(srs);
            if (typeof window !== "undefined") {
              window.dispatchEvent(new CustomEvent('mock-realtime-scores', { detail: { new: newScore } }));
            }
            ...
            const participations = getMockDrawParticipation();
            const createdObj = {
              id: data.id || "mock-part-" + Math.random().toString(36).substring(2, 15),
              user_id: data.user_id,
              draw_id: data.draw_id,
              status: data.status,
              created_at: new Date().toISOString(),
            ...
            participations.push(createdObj);
            saveMockDrawParticipation(participations);

            if (typeof window !== "undefined") {
              window.dispatchEvent(new CustomEvent('mock-realtime-draw-participation', { detail: { new: createdObj } }));
            }

            const chain = {
              select: () => ({
                single: async () => ({ data: createdObj, error: null }),
                maybeSingle: async () => ({ data: createdObj, error: null }),
                then: async (resolve) => resolve({ data: [createdObj], error: null })
              }),
              single: async () => ({ data: createdObj, error: null }),
              maybeSingle: async () => ({ data: createdObj, error: null }),
              then: async (resolve) => resolve({ data: [createdObj], error: null })
        ...
        upsert: (data, options) => {
          ...
          if (table === "draw_participation") {
            const participations = getMockDrawParticipation();
            const payload = Array.isArray(data) ? data : [data];
            const updatedList = [...participations];
            const results = [];

            payload.forEach(item => {
              const idx = updatedList.findIndex(p => p.user_id === item.user_id && p.draw_id === item.draw_id);
              const obj = {
                id: item.id || (idx !== -1 ? updatedList[idx].id : "mock-part-" + Math.random().toString(36).substring(2, 15)),
                user_id: item.user_id,
                draw_id: item.draw_id,
                status: item.status,
                created_at: idx !== -1 ? updatedList[idx].created_at : new Date().toISOString(),
                updated_at: new Date().toISOString()
              };
              if (idx !== -1) {
                updatedList[idx] = obj;
            ...
            saveMockDrawParticipation(updatedList);
            createdObj = Array.isArray(data) ? results : results[0];
          ...
          const chain = {
            select: () => ({
              single: async () => ({ data: Array.isArray(createdObj) ? createdObj[0] : createdObj, error: upsertError }),
              maybeSingle: async () => ({ data: Array.isArray(createdObj) ? createdObj[0] : createdObj, error: null }),
              then: async (resolve) => resolve({ data: Array.isArray(createdObj) ? createdObj : [createdObj], error: upsertError })
            }),
            single: async () => ({ data: Array.isArray(createdObj) ? createdObj[0] : createdObj, error: upsertError }),
            maybeSingle: async () => ({ data: Array.isArray(createdObj) ? createdObj[0] : createdObj, error: null }),
            then: async (resolve) => resolve({ data: Array.isArray(createdObj) ? createdObj : [createdObj], error: upsertError })
              ...
              if (table === "profiles") {
                ...
                if (!updatedSub && idx !== -1) {
                  updatedSub = {
                    ...
                    role: (users[idx].email.startsWith("admin@") || users[idx].email.includes("admin")) ? "admin" : "user",
                    stripe_customer_id: users[idx].stripe_customer_id || null,
                    pref_system_updates: users[idx].pref_system_updates !== false,
                    pref_draw_results: users[idx].pref_draw_results !== false,
                    pref_winner_alerts: users[idx].pref_winner_alerts !== false,
                    created_at: new Date().toISOString()
                  };
                }
                if (typeof window !== "undefined" && updatedSub) {
                  window.dispatchEvent(new CustomEvent('mock-realtime-profiles', { detail: { new: updatedSub } }));
                }
              } else if (table === "subscriptions") {
                const subs = getMockSubscriptions();
                ...
                if (idx !== -1) {
                  ...
                  if (typeof window !== "undefined") {
                    window.dispatchEvent(new CustomEvent('mock-realtime-subscriptions', { detail: { new: updatedSub } }));
                  }
                }
              } else if (table === "scores") {
                const srs = getMockScores();
                ...
                if (idx !== -1) {
                  ...
                  updatedSub = srs[idx];
                  if (typeof window !== "undefined") {
                    window.dispatchEvent(new CustomEvent('mock-realtime-scores', { detail: { new: updatedSub } }));
                  }
                }
              } else if (table === "user_charity_selections") {
                const sels = getMockSelections();
                ...
                const participations = getMockDrawParticipation();
                const idx = participations.findIndex(s => s[field] === value);
                if (idx !== -1) {
                  participations[idx] = { ...participations[idx], ...updates, updated_at: new Date().toISOString() };
                  saveMockDrawParticipation(participations);
                  updatedSub = participations[idx];

                  if (typeof window !== "undefined") {
                    window.dispatchEvent(new CustomEvent('mock-realtime-draw-participation', { detail: { new: updatedSub } }));
                  }
                }
              }
        ...
        delete: () => {
          ...
          const chain = {
            ...
            then: async (resolve) => {
              ...
              if (table === "scores") {
                tableData = getMockScores();
                saveFunc = (updated) => {
                  saveMockScores(updated);
                  if (typeof window !== "undefined") {
                    deleteFilters.forEach(f => {
                      window.dispatchEvent(new CustomEvent('mock-realtime-scores', { detail: { old: { [f.field]: f.value } } }));
                    });
                  }
                };
              } else if (table === "user_charity_selections") {
                tableData = getMockSelections();
                saveFunc = saveMockSelections;
              } else if (table === "charities") {
                tableData = getMockCharities();
                saveFunc = (updated) => localStorage.setItem("fundora-mock-charities", JSON.stringify(updated));
              } else if (table === "draws") {
                tableData = getMockDraws();
                saveFunc = saveMockDraws;
              } else if (table === "draw_entries") {
                tableData = getMockDrawEntries();
                saveFunc = (updated) => {
                  saveMockDrawEntries(updated);
                  if (typeof window !== "undefined") {
                    deleteFilters.forEach(f => {
                      window.dispatchEvent(new CustomEvent('mock-realtime-draw-entries', { detail: { old: { [f.field]: f.value } } }));
                    });
                  }
                };
              } else if (table === "draw_participation") {
                tableData = getMockDrawParticipation();
                saveFunc = (updated) => {
                  saveMockDrawParticipation(updated);
                  if (typeof window !== "undefined") {
                    deleteFilters.forEach(f => {
                      window.dispatchEvent(new CustomEvent('mock-realtime-draw-participation', { detail: { old: { [f.field]: f.value } } }));
                    });
                  }
                };
              }
              ...
              const remaining = tableData.filter(item => {
                const matchesAll = deleteFilters.every(f => item[f.field] === f.value);
                return !matchesAll;
              });
...
export const createClient = () => {
  const { url, anonKey, isPlaceholder: isMock } = getSupabaseCredentials();
  if (isMock) {
