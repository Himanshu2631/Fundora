  ...
  return {
    ...
    from: (table) => {
      ...
      const getMockPayments = () => {
        if (typeof window === "undefined") return [];
        try {
          return JSON.parse(localStorage.getItem("fundora-mock-payments") || "[]");
        } catch {
          return [];
        }
      };
      ...
      return {
        select: (fields) => {
          ...
          if (table === "profiles") {
            ...
            matchedData = [
              ...sessionUser,
              ...mockUsers.map(u => ({ id: u.id, email: u.email, full_name: u.full_name, role: "user", stripe_customer_id: u.stripe_customer_id || null })),
              ...staticMockUsers
            ];
