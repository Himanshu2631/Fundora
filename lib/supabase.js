    ...
    from: (table) => {
      ...
      const getMockDraws = () => {
        ...
        try {
          ...
          setCookie("fundora-mock-draws", JSON.stringify(seed));
          return seed;
        } catch {
          return [];
        }
      ...
      const getMockClaims = () => {
        ...
        try {
          ...
          setCookie("fundora-mock-winner-claims", JSON.stringify(seed));
          return seed;
        } catch {
          return [];
        }
          ...
          return {
            eq: (field, value) => {
              if (table === "scores") {
                ...
                if (typeof window !== "undefined") {
                  window.dispatchEvent(new CustomEvent('mock-realtime-scores', { detail: { old: { [field]: value } } }));
                }
              } else if (table === "user_charity_selections") {
                const sels = getMockSelections();
