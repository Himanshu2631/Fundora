...
const createMockServer = (cookieStore) => {
  ...
  return {
    ...
    from: (table) => {
      return {
        select: (fields) => {
          const makeChain = (data, totalCount = null) => {
            const initialCount = totalCount !== null ? totalCount : data.length;
            const chain = {
              eq: (field, value) => {
              ...
              order: (sortField, sortOptions) => {
                ...
                return makeChain(sorted, initialCount);
              },
              range: (from, to) => {
                const sliced = data.slice(from, to + 1);
                return makeChain(sliced, initialCount);
              },
              limit: (limitVal) => {
                return makeChain(data.slice(0, limitVal), initialCount);
              },
              maybeSingle: async () => ({ data: data[0] || null, error: null }),
              single: async () => ({ data: data[0] || null, error: data[0] ? null : { message: "Not found" } }),
              then: async (resolve) => resolve({ data, error: null, count: initialCount })
            };
            return chain;
          };
          ...
          if (table === "profiles") {
            ...
            matchedData = getCookieList("fundora-mock-draw-participation");
          } else if (table === "campaign_viability_assessments") {
            matchedData = getCookieList("fundora-mock-assessments");
          } else if (table === "charities") {
            matchedData = getCookieList("fundora-mock-charities", seedCharities);
          }
        ...
        insert: (data) => {
          ...
          if (table === "subscriptions") {
            ...
            const participations = getCookieList("fundora-mock-draw-participation");
            createdObj = {
            ...
            saveCookieList("fundora-mock-draw-participation", participations);
          } else if (table === "campaign_viability_assessments") {
            const assessments = getCookieList("fundora-mock-assessments");
            const assessmentType = data.assessment_type || "initial_48h";

            if (assessmentType === "initial_48h" && assessments.some(a => a.campaign_id === data.campaign_id && (a.assessment_type === "initial_48h" || !a.assessment_type))) {
              insertError = { message: 'duplicate key value violates unique constraint "idx_unique_campaign_initial_48h_assessment"', code: "23505" };
            } else {
              createdObj = {
                id: data.id || "mock-assess-" + Math.random().toString(36).substring(2, 15),
                campaign_id: data.campaign_id,
                assessment_type: assessmentType,
                risk_probability: data.risk_probability,
                viability_score: data.viability_score,
                risk_level: data.risk_level,
                prediction_horizon_hours: data.prediction_horizon_hours || 48,
                model_name: data.model_name || "Random Forest Champion",
                model_type: data.model_type || "RandomForestClassifier",
                n_features: data.n_features || 56,
                calibration: data.calibration || "Platt Scaling (Sigmoid)",
                base_rate_risk: data.base_rate_risk || 0.4996,
                top_risk_factors: data.top_risk_factors || [],
                top_supporting_factors: data.top_supporting_factors || [],
                detailed_risk_factors: data.detailed_risk_factors || [],
                detailed_supporting_factors: data.detailed_supporting_factors || [],
                research_disclaimer: data.research_disclaimer || null,
                created_at: data.created_at || new Date().toISOString(),
                updated_at: data.updated_at || new Date().toISOString()
              };
              assessments.push(createdObj);
              saveCookieList("fundora-mock-assessments", assessments);
            }
          }
        ...
        upsert: (data, options) => {
          ...
          if (table === "draw_participation") {
            ...
            saveCookieList("fundora-mock-draw-participation", updatedList);
        ...
        delete: () => {
          ...
          const chain = {
            ...
            then: async (resolve) => {
              ...
              if (table === "scores") {
                tableData = getCookieList("fundora-mock-scores");
                saveFunc = (updated) => saveCookieList("fundora-mock-scores", updated);
              } else if (table === "draws") {
                tableData = getCookieList("fundora-mock-draws", seedDraws);
                saveFunc = (updated) => saveCookieList("fundora-mock-draws", updated);
              } else if (table === "draw_entries") {
                tableData = getCookieList("fundora-mock-draw-entries");
                saveFunc = (updated) => saveCookieList("fundora-mock-draw-entries", updated);
              } else if (table === "draw_participation") {
                tableData = getCookieList("fundora-mock-draw-participation");
                saveFunc = (updated) => saveCookieList("fundora-mock-draw-participation", updated);
              } else if (table === "campaign_viability_assessments") {
                tableData = getCookieList("fundora-mock-assessments");
                saveFunc = (updated) => saveCookieList("fundora-mock-assessments", updated);
              }
...
export const createServer = async (customCookieStore = null) => {
