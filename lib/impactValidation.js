/**
 * Validate an impact score entry.
 * @param {object} params
 * @param {number|string} params.score - The impact score value.
 ...
 */
export function validateImpactScore({ score, scoreDate, existingScores }) {
  if (score === undefined || score === null || score === "") {
    return { isValid: false, error: "Impact score is required." };
  }
  
  const scoreNum = Number(score);
  if (isNaN(scoreNum) || !Number.isInteger(scoreNum) || scoreNum < 1 || scoreNum > 45) {
    return { isValid: false, error: "Impact score must be an integer between 1 and 45." };
  }
  ...
  if (existingScores && Array.isArray(existingScores)) {
    ...
    if (isDuplicate) {
      return { isValid: false, error: "An impact score entry already exists for this date." };
    }
  }
...
export const validateScore = validateImpactScore;
