...
export function validateDrawEntry(userGivingScore, drawMinScore, subscriptionStatus) {
  ...
  if (isNaN(scoreNum) || scoreNum < minScoreNum) {
    return { 
      isValid: false, 
      error: `Your Impact Score (${scoreNum} pts) does not meet the minimum requirement for this draw (${minScoreNum} pts).` 
    };
  }
  return { isValid: true };
}
 ...
 * Checks active subscription status, at least one registered impact score, and at least one charity allocation.
...
/**
 * Validate a user's winner claim submission.
 * Checks for screenshot URL presence.
 * @param {object} claim - Claim payload (screenshot_url, entry_id).
 * @returns {object} { isValid: boolean, error?: string }
 */
export function validateWinnerClaim(claim) {
  if (!claim) {
    return { isValid: false, error: "Claim data is required." };
  }
  if (!claim.entry_id || typeof claim.entry_id !== "string" || claim.entry_id.trim() === "") {
    return { isValid: false, error: "Winning entry ID is required." };
  }
  if (!claim.screenshot_url || typeof claim.screenshot_url !== "string" || !claim.screenshot_url.startsWith("http")) {
    return { isValid: false, error: "A valid screenshot verification URL is required." };
  }
  return { isValid: true };
}

/**
 * Validate claim review transitions.
 * Verification States: pending, approved, rejected, paid
 * @param {string} status - Target claim verification status.
 * @returns {object} { isValid: boolean, error?: string }
 */
export function validateClaimReview(status) {
  const validStates = ["pending", "approved", "rejected", "paid"];
  if (!validStates.includes(status)) {
    return { 
      isValid: false, 
      error: `Invalid claim verification status. Must be one of: ${validStates.join(", ")}.` 
    };
  }
  return { isValid: true };
}
